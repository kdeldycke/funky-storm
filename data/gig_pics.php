<?
  ...
  $gig_photo_dir = 'gigs/'.$date.'/';
	...
	if(!isset($img) || !file_exists($gig_photo_dir.$img.'.jpg')) {  //si $img n'existe pas ou si elle n'est pas valide, on affiche la totale des photos
	  $txt .= "<center><a href=\"photos.php\">retour à la liste des \"Evénements Funky\"</a></center><br><br>" ;
    ...
    //on stocke la liste des fichiers dans un tableau et on le tri
      ...
      if(($file!='.') && ($file!='..') && (!is_dir($gig_photo_dir.$file)) && (substr($file, strlen($file)-4, 4)=='.jpg')) //si c'est un fichier .jpg, on prend
		...
		$txt .= "</table>" ;
		$txt .= "<center><b>Note</b>&nbsp;: les commentaires appartiennent à leurs auteurs (ceux qui les ont postés). Nous n'en sommes pas responsables.</center>" ;
  	...
  	$txt .= "<br><center>";
		if(isset($prev))
      $txt .= "<a href=\"data/gig_pics.php?date=$date&img=$prev\">photo précédente</a>";
		else
      $txt .= "photo précédente";
		$txt .= " | <a href=\"photos.php\">retour à la liste des \"Evénements Funky\"</a> | <a href=\"data/gig_pics.php?date=$date\">retour à la liste des photos</a> | ";
		if(isset($next))
	  ...
	  if(!file_exists($comt_path)) { //si fichier de données perso inexistant
        ...
        $txt .= "posté par " ;
				...
				$txt .= " le $comment_item[0]" ;  //écriture de la signature
